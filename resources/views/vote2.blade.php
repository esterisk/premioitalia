	...
	<div class="row row-categoria">
		<div class="col-sm-12">
			...
			<div class="finalisti">
				...
				@endforeach
				<button type="button" class="btn btn-block btn-outline-danger{{ !empty($voto) && ($voto->$slug == 'N') ? ' active' : ''}}" data-value="N" data-cat="{{ $cat->slug }}">Nessun finalista merita il premio per questa categoria</button>
				<button type="button" class="btn btn-block btn-outline-warning{{ empty($voto) || ($voto->$slug == 0) ? ' active' : ''}}" data-value="0" data-cat="{{ $cat->slug }}">Non voto per questa categoria</button>
			</div>
		</div>
	</div>
	@endforeach

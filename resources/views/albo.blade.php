			...
			<div class="card-body">
				<form class="form-inline">
					...
					<select class="form-control form-control-sm my-1 mr-sm-2" onchange="location=this.options[this.selectedIndex].value">
						...
						@foreach (\App\Models\Raggruppamento::all() as $r)
			...
			<div class="card-body">
				<form class="form-inline">
					...
					<select class="form-control form-control-sm my-1 mr-sm-2" onchange="location=this.options[this.selectedIndex].value">
						...
						@foreach (\App\Models\Albo::distinct('anno')->orderBy('anno','desc')->pluck('anno') as $a)
...
<div class="row albo-anno">
	...
	<div class="col-sm-11">
		<?php $con = \App\Models\Convention::whereAnno($winner['anno'])->where('italcon','>',0)->first(); ?>
		@if ($con)
		<h2>{{ $con->titolo_edizione ? $con->titolo_edizione.' - ' : '' }}Italcon {{ $con->italcon }}</h2>
		<p class="convention">{{ $con->city }}{{ $con->premiazione ? ', '._date($con->premiazione, 'd M Y') : '' }}</p>
		@else

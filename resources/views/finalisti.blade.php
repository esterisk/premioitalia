	...
	@foreach ($categorie as $categoria)
		<div class="row">
			<div class="col-sm-12">
				...
				<dl>
					<dt>Votanti prima fase</dt> <dd>{{ $categoria->conteggio->votanti }} ({{ $categoria->conteggio->percentuale_votanti }}% di {{ $categoria->conteggio->totale_votanti }} totali)</dd>
				...
				@if ($annata->risultati())
					...
					<dt>Finalisti</dt> <dd>{{ $categoria->conteggioFinale->finalisti  - 1 }}</dd>
				@endif
				</dl>
				...
				<p>Sul podio</p>
				<ul>
				@foreach ($categoria->podio($annata->anno)->get() as $vincitore)
					<li>{{ $vincitore->posizione }}° classificato: <strong>{!! $vincitore->descrizione_ricca() !!}</strong></li>
				@endforeach
				</ul>
				@endif

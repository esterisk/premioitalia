	...
	<div class="row row-categoria">
		<div class="col-sm-12">
			...
			<p class="definizione">{!! str_replace('periodo', ($annata->anno - 1), $cat->definizione) !!}
				<button class="btn btn-link btn-sm rule-detail">Dettagli</button>
			</p>
			@if($cat->esclusi_ultimi > 0) <p>Non sono candidabili i vincitori delle ultime {{ $cat->esclusi_ultimi }} edizioni:</p>
			<ul>
			...
			<div class="candidature-env">
				...
				<div class="collapse" id="candidature-{{ $cat->slug }}">
					<div class="card card-body">
						...
						<p><small>
								Le candidature spontanee sono solo suggerimenti proposti da autori e lettori.<br />
								Le candidature sono in ordine casuale ogni volta diverso. La validità della candidatura è verificata ma non assicurata.<br />
								<strong>L’elettore è libero di inserire segnalazioni non comprese tra le candidature spontanee.</strong>
							</small></p>
					</div>
				</div>
			</div>
			<div class="segnalazioni">
				<table class="table">
					<tbody>
						...
						@for ($i=1; $i<=\Config::get('premioitalia.numero_segnalazioni'); $i++) <tr>
							<th class="letter">{{ substr("ABCDEF",$i-1,1) }}</th>
							@foreach ($cat->campi as $c)
							<?php $label = $cat->fieldLabel($i,$c); ?>
							<td><input type="text" data-riga-segnalazione="{{ $cat->slug }}-{{ $i }}" class="form-control" value="{{ !empty($voto) ? $voto->$label : '' }}" title="{{ Config::get('premioitalia.campi.'.$c) }}" placeholder="{{ Config::get('premioitalia.campi.'.$c) }}"
									name="{{ $label }}"></td>
							@endforeach

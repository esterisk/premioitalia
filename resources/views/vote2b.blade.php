...
<div class="row">
	<div class="col-sm-12">
		...
		<p>Per ogni categoria puoi scegliere fino a tre candidati che consideri i più meritevoli. Se pensi che nessuno dei finalisti sia meritevole e che il premio non debba essere assegnato, scegli «Non assegnare il premio per questa categoria». Se non conosci i candidati e preferisci non votare
			per una categoria, scegli «Non voto per questa categoria».</p>
...
<form id="vote-form" action="{{ route('vote.store') }}" method="post">
	...
	<style>
		...
		.finalisti button.choice-1 {
			right: 75px;
		}

		.finalisti button.choice-2 {
			right: 40px;
		}

		.finalisti button.choice-3 {
			right: 5px;
		}

		.finalisti button.choice-1.active {
			opacity: 1;
			background: #090;
		}

		.finalisti button.choice-2.active {
			opacity: 1;
			background: #6C6;
		}

		.finalisti button.choice-3.active {
			opacity: 1;
			background: #CC6;
		}
	</style>
	...
	<div class="row row-categoria">
		<div class="col-sm-12">
			...
			<div class="finalisti" data-cat="{{ $cat->slug }}">
				...
				@endforeach
				<div style="position:relative">
					<button type="button" class="activable btn btn-block btn-outline-danger{{ ($user->choice($voto, $slug, 1) === 'N') ? ' active' : ''}}" data-value="N" data-cat="{{ $cat->slug }}">Nessun finalista merita il premio per questa categoria</button>
				</div>
				<div style="position:relative">
					<button type="button" class="activable btn btn-block btn-outline-dark{{ ($user->choice($voto, $slug, 1) === 0 || $user->choice($voto, $slug, 1) == 'U') ? ' active' : ''}}" data-value="U" data-cat="{{ $cat->slug }}">Preferisco non votare per questa categoria</button>
				</div>
			</div>
		</div>
	</div>

...
class VoteController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        switch ($this->annata->fase()) {
            ...
            case 'fase1':
                ...
                ray($response)->label('save1 response');
                if ($request->ajax()) {
                    ray('request is ajax')->green();

                    return response()->json($response);
                } else {
                    ray('request is not ajax')->red();
    ...
    private function vote1($response = false)
    {
        ...
        if ($voto) {
            if ($voto->stato == 'preparazione') {
                $data['voto'] = json_decode($voto->dati);
            } else {
    ...
    private function vote2($response = false)
    {
        ...
        if ($voto) {
            if ($voto->stato == 'preparazione') {
                $data['voto'] = json_decode($voto->dati, true);
            } else {
                $data['stato'] = $voto->stato;
                $data['sent_at'] = $voto->sent_at;
            }
    ...
    {
        if (! ($user = auth()->user()) || ($user->admin <= 0)) {
            abort(403);
        }

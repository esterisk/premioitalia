...
class Australian
{
    ...
    public function calcolaVincitori($save = false, $anno = false)
    {
        ...
        if (! $this->caricaAnnata($anno)) {
            return false;
        }

        $result = [];
        $totale_votanti = $this->totaleVotanti();

        foreach ($categorie as $categoria) {
            $result[$categoria->getKey()] = $this->calcolaCategoria($categoria, $save, $totale_votanti);
        }
    ...
    /**
     * Carica l'annata da elaborare, per anno o corrente.
     */
    public function caricaAnnata($anno = false): ?Annata
    {
        if (! $anno) {
            $this->annata = Annata::corrente();
        } else {
            $this->annata = Annata::whereAnno($anno)->first();
        }
    ...
    /**
     * Numero di elettori che hanno inviato il voto di seconda fase.
     */
    public function totaleVotanti(): int
    {
        return $this->annata->voti()->fase2()->inviato()->count();
    }

    /**
     * Esegue l'australian ballot su una singola categoria e restituisce il
     * conteggio finale (salvato solo se $save è true).
     */
    public function calcolaCategoria(Categoria|int $categoria, bool $save = false, ?int $totale_votanti = null): ConteggioFinale
    {
        if (! $this->annata) {
            $this->caricaAnnata();
        }
        if ($totale_votanti === null) {
            $totale_votanti = $this->totaleVotanti();
        }
        $categoria_id = $categoria instanceof Categoria ? $categoria->getKey() : $categoria;
        ...
        $elenco_finalisti = collect($this->candidati);
        ...
        if (isset($this->vincitori[0])) {
            $this->preferenze_vincitore = $this->validi[$this->vincitori[0]];
        }
        $this->conteggio->percentuale_votanti = $this->conteggio->totale_votanti > 0
            ? round($this->conteggio->votanti / $this->conteggio->totale_votanti * 100)
            : 0;
        if (isset($this->vincitori[0])) {
        ...
        if ($elenco_finalisti->count() > 1 && count($this->vincitori) < 3) {
            ...
            if ($elenco_finalisti->count() > 2 && (count($this->vincitori) + count($this->secondi) < 3)) {
                $this->caricaCandidati($categoria_id);
                $this->caricaVoti($categoria_id);
                $contoValidi = $this->validi->count();
        ...
        $this->conteggio->elenco_finalisti = $elenco_finalisti->implode(',');
        ...
        if ($save) {
            Candidato::whereAnno($this->annata->anno)->whereCategoriaId($categoria_id)->where('finalista', '>', 0)->update(['finalista' => 1, 'posizione' => 0]);
            $this->conteggio->save();
    ...
    {
        // untieOtherChoices() restituisce null quando non riesce a ridurre la parità.
        $candidates = $this->untieOtherChoices($candidates, $this->voti) ?? $candidates;
    ...
    public function contaVoti()
    {
        ...
        foreach ($this->voti as &$voto) {
            if (! empty($voto[0])) {
                $this->validi[$voto[0]] = $this->validi->get($voto[0]) + 1;
            }
        }
    ...
    public function rimuoviVotiEliminato($elimina)
    {
    ...
    public function caricaCandidati($categoria_id, $load = false)
    {
        if ($load) {
    ...
    public function caricaVoti($categoria_id, $load = false)
    {
        if ($load) {
            ...
            foreach ($this->annata->preferenze()
                ...
                if ($pref->candidato_1_id > 0) {
                    if ($pref->candidato_2_id) {
                        $scelte[] = $pref->candidato_2_id;
                    }
                    if ($pref->candidato_3_id) {
                        $scelte[] = $pref->candidato_3_id;
                    }
                }

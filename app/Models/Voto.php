...
class Voto extends Model
{
	...
	public function scopeAnnata($query, $annata = null)
	{
		if (!$annata) $annata = Annata::current();
		return $query->whereAnno($annata->anno);
	}

	public function scopeFase($query, $fase)
	{
		return $query->whereFase($fase);
	}
	public function scopeFase1($query)
	{
		return $query->whereFase('fase1');
	}
	public function scopeFase2($query)
	{
		return $query->whereFase('fase2');
	}
	public function scopeInviato($query)
	{
		return $query->whereStato('inviato');
	}

	public function scopePreparazione($query)
	{
		return $query->whereStato('preparazione');
	}
	...
	{
		$s0 = $string;
		$s1 = $string = preg_replace_callback('|([A-Z]{2,})|', function ($m) {
	...
	public function invia1($email = true)
	{
		...
		foreach (Categoria::attive() as $cat) {
			...
			if (!empty($voti[$cat->slug])) {
				for ($i = 1; $i <= config('premioitalia.numero_segnalazioni'); $i++) if (!empty($voti[$cat->slug][$i])) {
